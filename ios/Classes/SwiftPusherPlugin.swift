    ...
    public func subscribe(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        if let pusherObj = SwiftPusherPlugin.pusher {
            ...
            case "presence":
                ...
                channel = pusherObj.subscribeToPresenceChannel(
                    ...
                    onMemberAdded: {
                        ...
                        do {
                            ...
                            if let eventSinkObj = SwiftPusherPlugin.eventSink {
                                ...
                                if (SwiftPusherPlugin.isLoggingEnabled) {
                                    print("Pusher event: CHANNEL:\(channelName) EVENT:\("pusher:member_added") DATA:\(jsonString ?? "no data")")
                                }
                            }
                        }
                        catch {
                    ...
                    onMemberRemoved: {
                        ...
                        do {
                            ...
                            if let eventSinkObj = SwiftPusherPlugin.eventSink {
                                eventSinkObj(jsonString)
                                if (SwiftPusherPlugin.isLoggingEnabled) {
                                    print("Pusher event: CHANNEL:\(channelName) EVENT:\("pusher:member_removed") DATA:\(jsonString ?? "no data")")
                                }
                            }
                        }
                        catch {
    ...
    public func bind(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        do {
            ...
            if let channelObj = channel {
                ...
                SwiftPusherPlugin.bindedEvents[bindArgs.channelName + bindArgs.eventName] = channelObj.bind(eventName: bindArgs.eventName, callback: {
                    data in
                    do {
